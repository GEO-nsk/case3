...
def get_num_hexagons():
    """
    Function to get a number of hexagons.
    """
    right_num_hex = False
    ...
    while right_num_hex is False:
        try:
            ...
            user_num_hex = int(input())
    return user_num_hex
def draw_hexagon(x, y, side_length, border_color, fill_color):
    """
    Function to draw hexagon with parameters:
...
for itr in range(num_hex - (num_hex // 2)):  # create left part of the pattern
    if itr % 2 == 0:
...
for itr in range(num_hex // 2):  # create right part of the pattern
    if itr % 2 == 0:
        for itr2 in range(num_hex):  # creating individual hexagons in each row
            if itr2 % 2 == 0:
                draw_hexagon((itr2 * d_hex_len) + (d_hex_len / 2), (itr * side_len * 3) + (1.5 * side_len), side_len,
                             'black', fill_color1)
            else:
                draw_hexagon((itr2 * d_hex_len) + (d_hex_len / 2), (itr * side_len * 3) + (1.5 * side_len), side_len,
                             'black', fill_color2)
